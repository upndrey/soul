...
Calc.prototype.API = async function() {
    ...
    if(LangAndPos == "0 0") {
        alert("Ваш город рождения не найден\nПожалуйста, проверьте правильность написания, или попробуйте указать соседний крупный город для расчёта (из одного часового пояса)");
        return false;
    }
...
Calc.prototype.changeStampDom = function(id) {
    ...
    switch(id) {
        case 1:
            ...
            <br><br>Вы чувствуете себя обиженной женщиной, считающей этот мир враждебным и небезопасным местом. Вам постоянно кто-то должен, а личная жизнь напоминает забег по треугольнику Карпмана. Не любите смотреться в зеркало из-за постоянного недовольства своим телом, а лень, усталость и прокрастинация сопровождают Вас изо дня в день. В гневе безжалостны и напоминаете Малифисенту. Находясь рядом с Ведьмой в отрицательном проявлении, мужчины испытывают напряжение и дискомфорт. Ничто не может принести Вам чувство глубокого удовлетворения. 
...
Calc.prototype.astroAPI = async function(ts) {
    ...
    return fetch(`https://vibracii-dushi.tmweb.ru/server.php?d=${date.getDate()}&m=${date.getMonth() + 1}&y=${date.getYear()}&h=${date.getHours()}&mi=${date.getMinutes()}`)
    .then(
        (res)=> {
            console.log(res);
            return res.json();
